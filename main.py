...
def event_filter(plugin_event, Proc, message_type):
    if message_type == "group":
        if plugin_event.data.group_id == '653931825':
            if (plugin_event.data.message == "Test"
                    or plugin_event.data.message == "T"):
                plugin_event.reply(llonebot_api.send_like(2432115441, 10))
